...
Working document for branch `agent/reading-speed-optimization`. Not corpus content; exempt from the front-matter and orphan checks in `scripts/audit.ps1`.
...
| query | today | grep path | no-search path |
|---|---:|---:|---:|
| `$D018` | 25,778 B | ~7,700 B | ~16,850 B |
| sprite multiplexer | 23,188 B | ~5,980 B | ~14,580 B |

−70% with text search, −36% without. Leaf pages grow slightly in total (−26 KB of tails, +38 KB of front matter): the win is on the read path, not repo size.
